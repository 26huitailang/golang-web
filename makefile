...
docker-build-arm:
	docker run --rm -it -v "$(GOPATH)":/go -w /go/src/github.com/26huitailang/golang-web -e CGO_ENABLED=1 -e GOOS=linux -e GOARCH=arm golang:1.12 go build -o $(BINARY_ARM) -v
